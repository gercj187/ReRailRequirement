...
namespace ReRailRequirement
{
    ...
    static class Main
    {
        ...
        {
			if (RRR_Multiplayer.IsClient)
			{
				GUIStyle clientInfoStyle =
					new GUIStyle(GUI.skin.box);

				clientInfoStyle.wordWrap = true;

				GUILayout.Box(
					"MULTIPLAYER CLIENT\n" +
					"ReRail Requirement settings are controlled by the host.",
					clientInfoStyle,
        ...
        static void OnSaveGUI(UnityModManager.ModEntry modEntry)
        {
            if (RRR_Multiplayer.IsClient)
            {
                Debug.Log("[ReRailRequirement][MP] Client settings were not saved; host settings are authoritative.");
                return;
            }
            ...
            RRR_Multiplayer.BroadcastHostSettings();
        }
